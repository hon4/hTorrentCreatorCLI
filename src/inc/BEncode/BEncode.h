...
class BEncode {
	...
	private:
		...
		std::string BEncode_int_uint64_t(std::uint64_t i) {
                        return "i" + std::to_string(i) + "e";
                }
		...
		std::string BEncode_dict(std::map<std::string, std::any> d) {
			...
			for (const auto& x : d) {
				...
				if (x.second.type() == typeid(std::string)) {
					...
					ret += BEncode_int_uint32_t(std::any_cast<std::uint32_t>(x.second));
				} else if (x.second.type() == typeid(std::uint64_t)) {
					ret += BEncode_int_uint64_t(std::any_cast<std::uint64_t>(x.second));
				} else {
		...
		std::string BEncode_list(std::vector<std::any> l) {
			...
			for (auto x : l) {
				if (x.type() == typeid(std::string)) {
					...
					ret += BEncode_int_uint32_t(std::any_cast<std::uint32_t>(x));
				} else if (x.type() == typeid(std::uint64_t)) {
					ret += BEncode_int_uint64_t(std::any_cast<std::uint64_t>(x));
				} else {
